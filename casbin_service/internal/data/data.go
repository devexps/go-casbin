...
import (
	...
	"github.com/devexps/go-casbin/casbin_service/v2/internal/data/ent/migrate"
...
func NewData(logger log.Logger, entClient *ent.Client) (*Data, func(), error) {
	l := log.NewHelper(log.With(logger, "module", "casbin_service/data"))
	d := &Data{
		log: l,
		db:  entClient,
	}
...
func NewEntClient(logger log.Logger, cfg *conf.Bootstrap) *ent.Client {
	l := log.NewHelper(log.With(logger, "module", "casbin_service/data/ent"))
